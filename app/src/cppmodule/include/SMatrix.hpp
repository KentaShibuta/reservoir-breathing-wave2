...
class SMatrix{
    ...
    public:
        ...
        std::unique_ptr<std::vector<std::vector<double>>> generate_uniform_randomd(std::size_t row_size, std::size_t col_size, float scale);
        ...
        std::unique_ptr<std::vector<std::vector<double>>> generate_normal_distributiond(std::size_t row_size, std::size_t col_size, float mean = 0.0, float stddev = 1.0);
        ...
        std::unique_ptr<std::vector<double>> dotd (const std::vector<std::vector<double>> &mat, const std::vector<double> &vec);
